...
import {
  ...
  ArrowLeft, Package, X, Leaf, AlertCircle, ChevronDown,
} from 'lucide-react';
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .listing-card:hover { transform:translateY(-5px); box-shadow:0 20px 50px rgba(0,0,0,0.09); border-color:${G.green400}; }
  ...
  .shimmer-box {
    ...
    background-size:700px 100%; animation:shimmer 1.4s infinite linear; border-radius:10px;
  }

  .filter-select {
    width:100%; border:1.5px solid ${G.border}; border-radius:12px;
    padding:10px 14px; font-size:13px; font-family:'DM Sans',sans-serif;
    color:${G.ink}; background:${G.cream}; outline:none; transition:border-color 0.15s;
    appearance:none;
  ...
  .filter-label {
    display:block; font-size:11px; font-weight:600; color:${G.muted};
  ...
  .load-more-btn {
    ...
    color:${G.ink}; font-size:14px; font-weight:600;
    font-family:'DM Sans',sans-serif; cursor:pointer;
    transition:all 0.15s;
  }
  .load-more-btn:hover { border-color:${G.green500}; color:${G.green700}; background:${G.green50}; }
...
export default function BrowsePage() {
  ...
  const fetchListings = useCallback(async (pageNum: number, replace = false) => {
    try {
      ...
      const { listings: newListings, hasMore: more, total: t } = res.data;
  ...
  return (
    <div className="browse-body">
      ...
      <header style={{
        ...
        <div style={{ maxWidth:'1200px', margin:'0 auto', padding:'0 24px' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'16px', paddingTop:'16px', paddingBottom:'12px' }}>
            ...
            {!isLoading && !error && (
              <div style={{ padding:'6px 14px', borderRadius:'100px', background:G.green100, color:G.green800, fontSize:'12px', fontWeight:700 }}>
            ...
            <div className="search-wrap">
              ...
              {searchQuery && (
                <button onClick={() => setSearchQuery('')} style={{
                  ...
                  background:'none', border:'none', cursor:'pointer', color:G.muted, display:'flex', alignItems:'center',
                }}>
                  <X style={{ width:'15px', height:'15px' }} />
                </button>
              )}
            </div>
            <button onClick={() => setShowFilters(f => !f)} className={`filter-btn ${showFilters || hasFilters ? 'active' : ''}`}>
              <SlidersHorizontal style={{ width:'16px', height:'16px' }} />
              Filters
              {hasFilters && (
                <span style={{
                  width:'18px', height:'18px', borderRadius:'50%',
                  background:G.green600, color:'#fff', fontSize:'10px', fontWeight:700,
            ...
            <div style={{ display:'flex', gap:'8px', flexWrap:'wrap', paddingBottom:'12px' }}>
              {activeChips.map(chip => (
                <button key={chip.label} onClick={chip.clear} style={{
                  ...
                  background:G.green100, color:G.green800, border:`1px solid ${G.green200}`,
                  fontSize:'12px', fontWeight:600, cursor:'pointer',
                }}>
                  {chip.label} <X style={{ width:'11px', height:'11px' }} />
                </button>
              ))}
      ...
      <div style={{ maxWidth:'1200px', margin:'0 auto', padding:'28px 24px 64px', display:'flex', gap:'28px', alignItems:'flex-start' }}>
        ...
        {/* ── FILTER SIDEBAR ── */}
        <aside style={{ width:'252px', flexShrink:0, display:showFilters ? 'block' : 'none', position:'sticky', top:'160px' }}>
          <div style={{ background:G.white, border:`1px solid ${G.border}`, borderRadius:'20px', padding:'24px' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px' }}>
              <h2 style={{ fontFamily:"'Fraunces',serif", fontSize:'16px', fontWeight:600, color:G.charcoal }}>Filters</h2>
              {hasFilters && (
                <button onClick={clearFilters} style={{ fontSize:'12px', fontWeight:600, color:G.green700, background:'none', border:'none', cursor:'pointer' }}>
                  Clear all
                </button>
              )}
            </div>
            ...
            <div style={{ marginBottom:'20px' }}>
              ...
              <div style={{ display:'flex', flexWrap:'wrap', gap:'7px' }}>
                {CONDITIONS.map(c => {
                  ...
                  return (
                    <button key={c} onClick={() => setSelectedCondition(active ? '' : c)} style={{
            ...
            <div>
              ...
              <input type="range" min="0" max="2000" step="50" value={maxPrice}
                onChange={e => setMaxPrice(Number(e.target.value))} style={{ width:'100%', marginBottom:'6px' }} />
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {!isLoading && error && (
            <div style={{ padding:'48px 24px' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'12px', padding:'20px', borderRadius:'16px', background:'#fef2f2', border:'1px solid #fecaca', color:'#b91c1c', fontSize:'14px', marginBottom:'16px' }}>
                <AlertCircle style={{ width:'18px', height:'18px', flexShrink:0 }} />
                {error}
              </div>
              <button onClick={() => fetchListings(1, true)} style={{ padding:'10px 20px', borderRadius:'100px', background:G.green600, color:'#fff', border:'none', cursor:'pointer', fontSize:'13px', fontWeight:600 }}>
                Try again
              </button>
            </div>
          )}
          ...
          {!isLoading && !error && filtered.length === 0 && (
            <div style={{ padding:'80px 24px', display:'flex', flexDirection:'column', alignItems:'flex-start', gap:'12px' }}>
              <div style={{ width:'64px', height:'64px', borderRadius:'20px', background:G.sand, display:'flex', alignItems:'center', justifyContent:'center', animation:'float 3s ease-in-out infinite' }}>
              ...
              <p style={{ fontSize:'14px', color:G.muted, lineHeight:1.6, maxWidth:'340px' }}>
                {hasFilters ? 'Try loosening your filters — there might be something close.' : 'Your campus has no listings yet. Be the first to post something!'}
              </p>
              {hasFilters && (
                <button onClick={clearFilters} style={{ padding:'10px 22px', borderRadius:'100px', background:G.green600, color:'#fff', border:'none', cursor:'pointer', fontSize:'13px', fontWeight:600, marginTop:'4px' }}>
                  Clear all filters
                </button>
              )}
            </div>
            ...
            <>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'20px' }}>
                {filtered.map((listing, idx) => {
                  ...
                  return (
                    <Link key={listing._id} to={`/item/${listing._id}`} className="listing-card" style={{ animationDelay:`${idx * 0.04}s` }}>
                      <div style={{ aspectRatio:'1', overflow:'hidden', background:G.sand, position:'relative' }}>
                        {listing.image ? (
                          <img src={listing.image} alt={listing.title} className="card-img" loading="lazy" />
                        ) : (
                          <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center' }}>
                            <Package style={{ width:'36px', height:'36px', color:'#c4b99a' }} />
                          </div>
                        )}
                        <div style={{ position:'absolute', top:'10px', left:'10px', padding:'4px 10px', borderRadius:'100px', background:cs.bg, color:cs.color, fontSize:'11px', fontWeight:700, letterSpacing:'0.04em' }}>
                          {listing.condition}
                        </div>
                        {listing.category && (
                          <div style={{ position:'absolute', top:'10px', right:'10px', padding:'4px 10px', borderRadius:'100px', background:'rgba(250,249,246,0.92)', color:G.muted, fontSize:'11px', fontWeight:600, backdropFilter:'blur(8px)' }}>
                            {listing.category}
                          </div>
                        )}
                      </div>
                      <div style={{ padding:'16px' }}>
                        <h3 style={{ fontSize:'14px', fontWeight:600, color:G.charcoal, marginBottom:'4px', lineHeight:1.35, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                          {listing.title}
                        </h3>
                        <p style={{ fontSize:'12px', color:G.muted, lineHeight:1.5, marginBottom:'12px', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' } as React.CSSProperties}>
                          {listing.description}
                        </p>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:'8px', paddingTop:'12px', borderTop:`1px solid ${G.border}` }}>
                          <div style={{ width:'26px', height:'26px', borderRadius:'50%', background:G.green100, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:'11px', fontWeight:700, color:G.green800 }}>
              ...
              {hasMore && (
                ...
                >
                  {isLoadingMore ? (
                    <span style={{ width:'16px', height:'16px', border:`2px solid ${G.border}`, borderTopColor:G.green600, borderRadius:'50%', animation:'shimmer 0.7s linear infinite', display:'inline-block' }} />
                  ) : (
